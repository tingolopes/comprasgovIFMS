...
CONFIG_APIS = {
    "LEGADO": {
        "base_url": BASE_URL,
        "pasta_cache": "temp/compras_legado",
    ...
    "LEI14133": {
        "base_url": BASE_URL,
        "pasta_cache": "temp/compras_14133",
...
PIPELINE_CONFIG = {
    ...
    "backoff_inicial": 2,
    "backoff_tentativas": 2,      # 2s → 4s

    # Cache: dias antes de re-verificar contratos ainda em aberto (PNCP)
    "dias_validade_cache_pncp": 7,

    # Log: a cada N skips, imprime um resumo (evita flood no terminal)
    "log_intervalo_skip": 50,
}

# Situações finais no PNCP (não precisam ser re-consultadas)
SITUACOES_FINAIS_PNCP = {3, 4, 5}

# ---------------------------------------------------------------------------
# MODALIDADES
# Dicionário oficial — código (int) → nome padronizado.
# Usado pelo transformer para garantir nomes consistentes,
# independente do texto livre retornado pela API.
# ---------------------------------------------------------------------------
